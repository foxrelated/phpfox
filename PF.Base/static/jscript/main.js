...
$Core.jsConfirm = function (params, doYes, doNo) {
    ...
    .dialog({
      ...
      modal: true,
      resizable: false,
      minWidth: $Core.exists('.admincp-fixed-menu') ? 550 : 300,
  ...
  updateCustomList: function() {
    ...
    if (!$iCnt) {
      ...
      checkedCustomList.each(function() {
        $($Core.Privacy.sCustomPrivacySelector).
          append('<div><input type="hidden" name="val' +
...
$Core.CoverPhoto = {
  ...
  submitForm: function() {
    ...
    if (this.maxUploadFileSize &&
      $('#global_attachment_photo_file_input')[0].files[0].size >
      this.maxUploadFileSize * 1000) {
      $Core.jsConfirm({
        ...
        if (myXhr.upload) {
          ...
          myXhr.upload.addEventListener('progress', function(e) {
            if (e.lengthComputable) {
              $('.progress-bar', '#uploading-cover').
                css({width: (Math.floor(e.loaded / e.total * 100)) + '%'});
            }
          }, false);
        }
        return myXhr;
  ...
  reposition: {
    ...
    cancel: function() {
      $('.pages_header_cover img, .profiles_banner_bg .cover img').
        draggable('destroy');
      $('.profiles_banner').removeClass('editing');
