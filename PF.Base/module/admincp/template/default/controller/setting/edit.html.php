...
<div class="panel panel-default global-settings _is_app_settings">
    ...
    <div id="{$aSetting.var_name}" class="form-group {if isset($aSetting.is_danger) && $aSetting.is_danger} has-warning {/if} {if !empty($aSetting.error)}has-error{/if} lines{if !empty($aSetting.group_class)} {$aSetting.group_class}{/if}{if !empty($aSetting.option_class)} is_option_class{/if}{if !empty($group_class) && !empty($aSetting.group_class) && $group_class != $aSetting.group_class} hidden{/if} clearfix"{if !empty($aSetting.option_class)} data-option-class="{$aSetting.option_class}"{/if}>
        ...
        <div class="custom-radio-wrapper">
            <label>
                <input name="val[value][{$aSetting.var_name}]" type="radio" {if $aSetting.value_actual == $mKey} checked{/if} value="{$mKey}" />
        ...
        <div class="item_is_active_holder">
            <span class="js_item_active item_is_active hide">
                <input type="radio" value="1" name="val[value][{$aSetting.var_name}]"{if $aSetting.value_actual == 1} checked="checked"{/if}>
            </span>
            <span class="js_item_active item_is_not_active hide">
                <input type="radio" value="0" name="val[value][{$aSetting.var_name}]"{if $aSetting.value_actual != 1} checked="checked"{/if}>
            </span>
        </div>
...
{/foreach}
    ...
    <div class="form-group lines form-group-save-changes">
        ...
        {if $sGroupId == 'mail'}
        <button type="submit" name="test" value="test" class="btn btn-primary">{_p var='Test and Save'}</button>
        {/if}
    </div>
    {/if}
</div>
