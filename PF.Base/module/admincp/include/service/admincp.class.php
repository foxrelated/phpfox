...
'link'  => $oUrl->makeUrl('admincp.block'),
    ...
    public function checkLatestLanguageVersions()
    {
        ...
        foreach ($aRows as $v) {
            $remote = json_decode(fox_get_contents(sprintf(Core\Home::store() . 'product/%d/view.json', $v['store_id'])), true);
        ...
        foreach (scandir($directory) as $name) {
            ...
            try {
                ...
                if ($info && isset($info['id']) && isset($info['name']) && isset($info['store_id']) && isset($info['version'])) {
                    ...
                    $remote = json_decode(fox_get_contents(sprintf(Core\Home::store() . 'product/%d/view.json', $info['store_id'])), true);
