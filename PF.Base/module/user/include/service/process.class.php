...
class User_Service_Process extends Phpfox_Service
{
    ...
    public function purchaseWithPoints($sModule, $iItem, $iTotal, $sCurrency, $iUserId = null)
    {
        ...
        if (isset($aSetting[$sCurrency]))
        {
            ...
            if ($iTotalPoints >= $iConversion)
            {
                ...
                $this->database()->update(Phpfox::getT('user_activity'), array('activity_points' => (int) $iNewPoints), 'user_id = ' . (int) $iUserId);
    ...
    public function add($aVals, $iUserGroupId = null)
    {
        ...
        if (Phpfox::getParam('user.profile_use_id') || (Phpfox::getParam('user.disable_username_on_sign_up') == 'full_name'))
        {
            ...
            if ($iCnt) {
                $this->database()->update($this->_sTable, array('user_name' => 'profile-' . uniqid()), 'user_id = ' . $iId);
            } else {
