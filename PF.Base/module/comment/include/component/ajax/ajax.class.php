...
class Comment_Component_Ajax_Ajax extends Phpfox_Ajax
{
            ...
            if ($mId == 'pending_moderation') {
                ...
                if (isset($aVals['is_via_feed'])) {
                    ...
                    if (isset($aVals['parent_id']) && $aVals['parent_id'] > 0) {
                        ...
                        $this->html('#js_comment_form_holder_' . $aVals['parent_id'], '');
                        $this->append('#js_comment_children_holder_' . $aVals['parent_id'], $this->getContent(false), '', false);
    ...
    public function updateText()
    {
        ...
        if (Phpfox::getService('comment.process')->updateText($this->get('comment_id'), $sTxt)) {
            ...
            if (Phpfox::getParam('core.allow_html')) {
                $sTxt = Phpfox::getLib('parse.output')->parse(Phpfox::getLib('parse.input')->prepare($sTxt), false);
            } else {
                $sTxt = Phpfox::getLib('parse.output')->parse($sTxt, false);
            }
            Phpfox::getLib('parse.output')->setImageParser(array('clear' => true));
            ...
            $this->html('#js_update_text_comment_' . $this->get('comment_id'),
                '<i>' . _p('last_update_on_time_stamp_by_full_name', array(
                    'time_stamp' => Phpfox::getTime(Phpfox::getParam('core.global_update_time'), PHPFOX_TIME),
    ...
    public function getText()
    {
        ...
        if (!isset($bHasPluginCall)) {
            $sTextareaId = "js_quick_edit" . $this->get('id');
            if ($this->get('simple')) {
                $this->call("$('#js_quick_edit_id" . $this->get('id') . "').html('<textarea data-component=\"comment-textarea\" name=\"quick_edit_input\" id=\"" . $sTextareaId . "\">" . Phpfox::getLib('parse.output')->ajax($aRow['text']) . "</textarea>');")
                    ->call('$Core.attachFunctionTagger(\'#js_quick_edit' . $this->get('id') . "');");
            } else {
                $this->call("$('#js_quick_edit_id" . $this->get('id') . "').html('<div id=\"sJsEditorMenu\" class=\"editor_menu\" style=\"display:block;\">' + Editor.setId('" . $sTextareaId . "').getEditor(true) + '</div><textarea data-component=\"comment-textarea\" name=\"quick_edit_input\" id=\"" . $sTextareaId . "\">" . Phpfox::getLib('parse.output')->ajax($aRow['text']) . "</textarea>');");
            }
            $this->call("\$Core.resizeTextarea($('#$sTextareaId'));")
                ->call("\$('.comment-edit-help-block').removeClass('hide');")
                ->call("\$Core.loadInit();");
        }
    }
    ...
    private function _getChildren($iId)
    {
        ...
        foreach ($aComments as $iKey => $aComment) {
            ...
            $this->template()->assign(array(
                    'aRow'           => $aComment,
                    'bCanPostOnItem' => ($iCacheCnt >= 1 ? false : true)
                )
            );
