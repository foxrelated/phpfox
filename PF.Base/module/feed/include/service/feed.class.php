...
class Feed_Service_Feed extends Phpfox_Service
{
	...
	public function getPhraseForLikes(&$aFeed, $bForce = false)
	{
		...
		if ((!isset($aFeed['likes']) && isset($oLike)) || (isset($oLike) && count($aFeed['likes']) > 2)) {
			$aFeed['likes'] = $oLike->getLikesForFeed($aFeed['type_id'], $aFeed['item_id'], false, 2, false, (isset($aFeed['feed_table_prefix']) ? $aFeed['feed_table_prefix'] : ''));
		...
		if (isset($aFeed['likes']) && is_array($aFeed['likes']) && $iCountLikes > 0) {
			...
			if (isset($aFeed['feed_is_liked']) && $aFeed['feed_is_liked'] && $iPhraseLimiter >= 2 && $aFeed['feed_total_like'] > $iPhraseLimiter) {
				...
				;
			} else if (isset($aFeed['feed_total_like']) && ($aFeed['feed_total_like'] > 2) && 2 != 1) {
...
//        }
	...
	public function getFeedActions($aFeed)
    {
        ...
        $aActions['can_share'] = Phpfox::isModule('share') && Phpfox::getUserParam('share.can_share_items') && !isset($aFeed['no_share']) && !empty($aFeed['type_id']) &&
