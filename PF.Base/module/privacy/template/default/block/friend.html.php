	...
	<div id="js_custom_list_actual_holder" class="privacy-custom-popup share-with" {if !count($aLists)}style="display:none"{/if}>
		<form class="form" method="post" action="#" onsubmit="return $Core.Privacy.updateCustomList();">
			...
			<div class="privacy-custom-button-wapper mt-2">
                <button role="button" class="btn btn-default mr-1" onclick="js_box_remove(this)">{_p var='cancel'}</button><input type="submit" class="btn btn-primary" value="{_p var='save'}" />
			</div>
		</form>
	</div>
			...
			<div>
				...
				<form class="form" method="post" action="#" onsubmit="$Core.Privacy.addList(this, event);">
					...
					<div class="privacy-custom-button-wapper">
                        <button role="button" class="btn btn-default mr-1" onclick="$Core.Privacy.hideCreateListForm()">{_p var='back'}</button><input type="submit" value="{_p var='create'}" class="btn btn-primary" />
                    </div>
				</form>
			</div>
		</div>
...
<script type="text/javascript">
  ...
  {/if}
    ...
    $Core.Privacy.phrases.custom_privacy = '{_p var='custom_privacy'}';
    $Core.Privacy.phrases.create_friends_list = '{_p var='create_friends_list'}';
