...
class Link_Service_Callback extends Phpfox_Service 
{
    ...
    public function getActivityFeed($aItem, $aCallBack = null, $bIsChildItem = false)
    {
        ...
        if (((defined('PHPFOX_IS_PAGES_VIEW') && defined('PHPFOX_PAGES_ITEM_TYPE') && !Phpfox::getService(PHPFOX_PAGES_ITEM_TYPE)->hasPerm($aRow['item_id'], 'link.view_browse_links'))
            || (!defined('PHPFOX_IS_PAGES_VIEW') && $aRow['module_id'] == 'pages' && !Phpfox::getService('pages')->hasPerm($aRow['item_id'],
                    'link.view_browse_links')))
        ) {
            return false;
        }
        ...
        $aReturn = array(
            ...
            'feed_link_comment' => Phpfox_Url::instance()->makeUrl($aItem['user_name'],
                array('link-id' => $aRow['link_id'])),
        ...
        if (Phpfox::getParam('core.warn_on_external_links')) {
            if (!preg_match('/' . preg_quote(Phpfox::getParam('core.host')) . '/i', $aRow['link'])) {
                $aReturn['feed_link_actual'] = Phpfox_Url::instance()->makeUrl('core.redirect',
                    array('url' => Phpfox_Url::instance()->encode($aRow['link'])));
                $aReturn['feed_title_extra_link'] = Phpfox_Url::instance()->makeUrl('core.redirect',
                    array('url' => Phpfox_Url::instance()->encode($aReturn['feed_title_extra_link'])));
            }
        }
        ...
        if (Phpfox::getParam('core.disable_all_external_urls') && !Phpfox::getService('link')->isInternalLink($aRow['link'])) {
