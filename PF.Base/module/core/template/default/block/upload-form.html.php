...
<div class="form-group js_upload_form_wrapper {if !empty($sCurrentPhoto)}show-current{/if}" id="js_upload_form_{$sType}_wrapper" data-type="{$sType}">
    ...
    {/if}
        ...
        <div class="{if !empty($aUploadCallback.style)}{$aUploadCallback.style}-{/if}dropzone-component dont-unbind" id="{$sType}-dropzone{if !empty($iId)}_{$iId}{/if}"
             data-component="dropzone"
             data-dropzone-id="{$sType}{if !empty($iId)}_{$iId}{/if}"
             data-url="{$aUploadCallback.upload_url}"
             data-param-name="{$aUploadCallback.param_name}"
             data-max-files="{$aUploadCallback.max_file}"
             data-clickable=".dropzone-button-{$sType}{if !empty($iId)}_{$iId}{/if}"
        ...
        >
            ...
            <div class="dz-default dz-message dropzone-inner">
                ...
                {/if}
                <div class="btn btn-primary dropzone-button dropzone-button-{$sType}{if !empty($iId)}_{$iId} dropzone-button-{$sType}{/if}">{_p var='browse_three_dot'}</div>
            </div>
            {else}
            ...
            <div class="dz-default {if empty($aUploadCallback.keep_form)}dz-message{/if}">
                ...
                <div class="dropzone-button-outer">
                    <div class="dropzone-button dropzone-button-{$sType}{if !empty($iId)}_{$iId} dropzone-button-{$sType}{/if}"><i class="{$aUploadCallback.upload_icon}"></i></div>
                </div>
                {/if}
                <div class="dropzone-content-outer">
                    ...
                    {if !empty($aUploadCallback.use_browse_button)}
                    <div class="btn btn-primary btn-gradient dropzone-button dropzone-button-{$sType}{if !empty($iId)}_{$iId} dropzone-button-{$sType}{/if}">{_p var='browse_three_dot'}</div>
                    {/if}
                </div>
            ...
            {if empty($aUploadCallback.keep_form)}
                <div class="dropzone-button outer dropzone-button-{$sType}{if !empty($iId)}_{$iId} dropzone-button-{$sType}{/if}">
                    <div class="inner">
