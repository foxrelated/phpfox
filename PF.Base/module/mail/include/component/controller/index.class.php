...
class Mail_Component_Controller_Index extends Phpfox_Component
{
	...
	public function process()
	{
        ...
        if (($aVals = $this->request()->getArray('val')) && isset($aVals['action'])) {
            if (isset($aVals['id'])) { //make sure there is at least one selected
                ...
                switch ($aVals['action']) {
                    ...
                    case 'delete':
                        ...
                        if (isset($aVals['select']) && $aVals['select'] == 'every') {
                            $aMail = Phpfox::getService('mail')->getAllMailFromFolder(Phpfox::getUserId(),
                                (int)$aVals['folder'], $bIsSentbox, $bIsTrash);
                            $aVals['id'] = $aMail;
                        }
            ...
            } else { // didn't select any message
        ...
        $this->search()->set(array(
                ...
                'search_tool' => array(
                    'table_alias' => 'm',
                    'search' => array(
                        'action' => $this->url()->makeUrl('mail',
                            array('view' => $this->request()->get('view'), 'id' => $this->request()->get('id'))),
        ...
        list($iCnt, $aRows, $aInputs) = Phpfox::getService('mail')->get($this->search()->getConditions(),
            $this->search()->getSort(), $this->search()->getPage(), $iPageSize, $bIsSentbox, $bIsTrash);
