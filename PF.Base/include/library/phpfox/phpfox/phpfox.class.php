...
class Phpfox
{
    ...
    {
        $active = \Phpfox::getLib('database')->select('is_active')
            ->from(':apps')
            ->where('apps_id="' . \Phpfox::getLib('parse.input')->clean($sAppId) . '"')
            ->executeField();
	...
	{
        (($sPlugin = Phpfox_Plugin::get('get_t')) ? eval($sPlugin) : false);
	...
	public static function run()
	{
		...
		} elseif (self::demoModeActive() && Phpfox::isAdmin() && strtolower(Phpfox_Request::instance()->get('req1')) !== 'flavors') {
        ...
        if (self::isAdminPanel()) {
            $oTpl->assign('sBodyClass', str_replace(['.', '/'], '-', Phpfox_Module::instance()->getFullControllerName()));
        }
...
//            while (ob_get_level()) ob_get_clean(); echo json_encode([$aLoadFiles, $aBundleFiles]); exit;
		...
		//	$content = (isset($blocks[2]) ? implode('', $blocks[2]) : '') . $content . (isset($blocks[4]) ? implode('', $blocks[4]) : '');
            ...
            $iNumberNotification = Phpfox::getService('core.helper')->shortNumberOver100((Phpfox::isUser() && Phpfox::isModule('friend'))  ? Phpfox::getService('notification')->getUnseenTotal() : -1);
            $iNumberMessage = Phpfox::getService('core.helper')->shortNumberOver100((Phpfox::isUser() && Phpfox::isModule('friend'))  ? Phpfox::getService('mail')->getUnseenTotal() : -1);
