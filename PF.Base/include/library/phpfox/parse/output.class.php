...
class Phpfox_Parse_Output
{
    ...
    public function parse($sTxt, $bParseNewLine = true)
    {
        ...
        $sTxt = preg_replace_callback('/\[PHPFOX_PHRASE\](.*?)\[\/PHPFOX_PHRASE\]/i', array($this, '_getPhrase'),
            $sTxt);
    ...
    private function _replaceHashTags($aMatches)
    {
        ...
        $sTxt = '<a href="' . Phpfox_Url::instance()->makeUrl('hashtag',
                array($sTagSearch)) . '" class="site_hash_tag">' . strip_tags($aMatches[0]) . '</a>';
...
//        $sTxt = trim(preg_replace("/(\s+)/", ' ', $sTxt));
    ...
    public function replaceHashTags($sTxt)
    {
        ...
        $sTxt = preg_replace_callback("/<a.*?<\/a>(*SKIP)(*F)|(\-?color:)\s*#([A-F0-9]{3,6})/i",
            array($this, '_replaceHexColor'), $sTxt);
    ...
    public function parseUrls($sTxt)
    {
        if (Phpfox::getParam('core.disable_all_external_emails')) {
            $sTxt = preg_replace_callback('/([\s>])*([.0-9a-z_+-]+)@(([0-9a-z-]+\.)+[0-9a-z]{2,})/i',
                array(&$this, '_replaceEmails'), $sTxt);
        }
        ...
        if (Phpfox::getParam('core.disable_all_external_urls')) {
            $sTxt = preg_replace_callback('/<a([^>]*) href="([^"]*)"([^>]*)>/i',
                array(&$this, '_replaceLinks'), $sTxt);
        }
        ...
        foreach ($aParts as $sPart) {
            if (substr($sPart, 0, 5) == 'href=') {
                ...
                if (!preg_match('/' . preg_quote(Phpfox::getParam('core.host')) . '/i', $sHref)) {
                    ...
                    $sHref = 'href="' . Phpfox_Url::instance()->makeUrl('core.redirect',
                            array('url' => Phpfox_Url::instance()->encode($sHref))) . '"';
                }
    ...
    public function split($sString, $iCount, $bBreak = false)
    {
        ...
        $sOut = ($bHasNonAscii === true ? str_replace(array('[PHPFOX_START]', '[PHPFOX_END]'), array('&lt;', '&gt;'),
    ...
    private function _replaceLinks($aMatches)
    {
        ...
        foreach ($aSites as $sSite) {
            ...
            if (preg_match('/' . str_replace('/', '\/', $sSite) . '/is', $aMatches[2])) {
                $href = $aMatches[2];
            }
        }
        ...
        if (!isset($href)) {
            $href = '#';
            $aMatches[1] = str_replace('target="_blank"', '', $aMatches[1]);
            $aMatches[3] = str_replace('target="_blank"', '', $aMatches[3]);
        }
